...
kubectl kustomize hack --load-restrictor='LoadRestrictionsRootOnly' | yq eval-all 'select((.kind == "CustomResourceDefinition" | not) and (.kind == "Namespace" | not))' - > helm/flux-app/templates/install.yaml
...
then
    ...
    sed -i "" -e "/image:/n;s/'{{/{{/g" -e "/image:/n;s/}}'/}}/g" helm/flux-app/templates/install.yaml
    sed -i "" -e "/'{{/n;s/{{\(.*\)'/{{\1/g" helm/flux-app/templates/install.yaml
fi
